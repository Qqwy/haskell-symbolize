...
import GHC.Exts (reallyUnsafePtrEquality#, UnliftedType,  Weak#, mkWeak#, makeStableName#, stableNameToInt#, Int(I#), deRefWeak#, ByteArray#)
import GHC.IO (IO(IO))
...
--
-- `hash` is a no-op and results in zero collissions, as `StableName`s are used.
--
